...
`;
  ...
  const processedDeviations = situation.map((item) => {
    // item.deviation dizisi mevcutsa ona map uygula
    if (item.Deviation) {
      return item.deviation.map((Deviation) => {
        // deviation üzerinde işlemler yapabilirsin
        return {
          AffectedDirection: Deviation.AffectedDirection,
          AffectedDirectionValue: Deviation.AffectedDirectionValue,
          // Deviation üzerinde daha fazla işlem yapabilirsin
        };
      });
    }
    // Eğer deviation yoksa boş dizi döndür
    return [];
  });
